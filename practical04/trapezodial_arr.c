#include<stdio.h>
#include<math.h>

//Function created to convert Degrees to radians
float degtorad(float degang);

int main(){

	int N=12, i;
	float ArrTan[N+1], deg, rad;
	
	//Calculate the values of f(xi) where i=0,1,....12
	//as array elements (xi in radians)
	for(i=0; i<=N; i++){
		deg=i*5.0;
		rad=degtorad(deg);
		ArrTan[i]=tan(rad);
		printf("ArrTan[%d]=%f\n", i, ArrTan[i]);
	}
	//Find area by trapezodial rule
	float area;
	area=ArrTan[0]+ArrTan[N];
	for(i=1; i<N; i++){
		area=area+2.0*ArrTan[i];
	}
	//Multiply area by (pi/3)/2N after converting it to radians
	float mult_rad=degtorad((60.0-0.0)/(2.0*N));
	area=mult_rad*area;

	//Aprroximated Result
	printf("\nTraperodial Result = %f\n", area);
	
	//Actual Result
	printf("\nReal Result = %f\n", log(2.0));
	
	return 0;
}

//Function to convert degree to radians
float degtorad(float degang){
	return((M_PI * degang)/180.0);
}
